...
public class JobSchedulingApp {
    public static void main(String[] args) {
        JobScheduler sch1 = new JobScheduler();
        Scanner scanner = new Scanner(System.in);
        boolean running = true;

        while (running) {
            System.out.println("\n=== Job Scheduling ===");
            System.out.println("0. Add Jobs"); // IN ORDER TO ADD SAMPLE DATA 
            ...
            scanner.nextLine(); 
            switch (choice) {
                case 0 -> {  // ADDS JOB TO PERFORM OTHER OPERATION
                    System.out.print("Enter job ID: ");
                    String jobId = scanner.nextLine();
                    sch1.addJob(jobId);
                }
                case 1 -> sch1.viewCompletedJobs();
                case 2 -> sch1.viewEstimatedCompletionTimes();
                case 3 -> sch1.scheduleBasedOnSystemLoad();
                case 4 -> sch1.simulateEmailNotification();
                case 5 -> sch1.validateInputs();
                case 6 -> sch1.cancelAllJobs();
                case 7 -> sch1.setJobDependencies(scanner);
                case 8 -> sch1.cancelUnneededJobs(scanner);
                case 9 -> sch1.viewLogs();
                case 10 -> sch1.exportSchedule();
                case 11 -> {
                ...
                scanner.nextLine(); // Clear the invalid user input
            }
        }
